...
**AMB-3** `P5` An ambulance is an ordinary car with two facts about it: it wears the service variant rather
...
- **AMB-4.3** `P5` It owes no stop to somebody **waiting** at an uncontrolled crossing, and it still owes one
...
- **AMB-4.5** `P5` A walker's escape from a crossing that never clears (PER-15) does not apply against its
...
**AMB-10** `P5` **A rescue is worked in human form.** An ambulance is stopped at a **standoff** short of the
